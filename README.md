...
- **`/complete [slug]`** — Re-verify against the code, fold anything durable into `CLAUDE.md`, and archive the task docs. Items only a human can verify (deploys, hardware runs, paid live calls) archive with an `ACCEPTANCE.md` checklist instead of stalling the task
