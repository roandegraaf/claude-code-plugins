...
- **One self-contained `.html` file.** Inline everything. To keep markup terse, you may use Tailwind's browser build (`<script src="https://unpkg.com/@tailwindcss/browser@4"></script>`); otherwise minimal inline CSS. No build step, no frameworks, no real images (use boxes/placeholders).
...
- Give each option `data-opt` and an `onclick="pick('<id>', this)"` with a short, meaningful id.
...
- **Auto-continue on click.** Right after opening the page, launch this as a **background Bash command** (`run_in_background: true`). It blocks until a *new* pick lands, prints the chosen id, then exits — which re-invokes you automatically, no message from the user needed:
  ```bash
  base=$(grep -c '__pick__' .mockups/.server.log 2>/dev/null); base=${base:-0}
  for i in $(seq 1 600); do
    n=$(grep -c '__pick__' .mockups/.server.log 2>/dev/null); n=${n:-0}
